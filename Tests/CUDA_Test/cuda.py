...
x_train = (x_train.astype(np.float64).reshape(-1,28,28,1)/255)[:100]
print(x_train.shape)
...
#model.train(x_train,y_train,optimizer=GradientDescentOptimizer(0.0001),cost_function="SquaredError",epoch=1000,print_cost_divisor=1,validation_X=x_test,validation_Y=y_test,minibatch_size=20,validation_divisor=10,minibatch_printcost= False)
model.train(x_train,y_train,optimizer=AdamOptimizer(0.001,epsilon=1e-8),cost_function="SquaredError",epoch=2000,print_cost_divisor=1,validation_X=x_test,validation_Y=y_test,minibatch_size=20,validation_divisor=5,minibatch_printcost= True)
